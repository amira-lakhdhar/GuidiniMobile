 ...
 * @author amira
 */
public class MofidierPharmacie extends BaseForm{
    Form current;

    public MofidierPharmacie(Resources res, Pharmacy rec) {
       super("Newsfeed", BoxLayout.y());
        Toolbar tb = new Toolbar(true);
        current = this;
        setToolbar(tb);
        getTitleArea().setUIID("container");
        setTitle("Modifier Pharmacy");
...
tb.addSearchCommand(e -> {
       ...
       // TextField LPD = new TextField(rec.getPrixlpd(), "LPD", 20, TextField.ANY);
         ...
         TextField name = new TextField(rec.getName(), "name", 20, TextField.ANY);
         TextField hourly = new TextField(rec.getHourly(), "hourly", 20, TextField.ANY);
         TextField phone = new TextField(rec.getPhone(), "name", 20, TextField.ANY);
         TextField email = new TextField(rec.getEmail(), "email", 20, TextField.ANY);

 
        name.setUIID("NewsTopLine");
        name.setSingleLineTextArea(true);

        hourly.setUIID("NewsTopLine");
        hourly.setSingleLineTextArea(true);
        ...
        btnModifier.addPointerPressedListener(l -> {
          rec.setName(name.getText());
              rec.setHourly(hourly.getText());
          rec.setPhone(phone.getText());
          rec.setEmail(email.getText());


            if (Service_Pharmacy.getInstance().ModifierPharmacie(rec)) {
